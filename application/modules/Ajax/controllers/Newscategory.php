...
class NewscategoryController extends BaseController
{
    ...
    public function saveAction() {
        ...
        if($id > 0) {
            //判断别名是否存在
            $ret = $this->loadModel('Newscategory',array(),'Admin')->isTrueName($data['nc_alias'],$id);
            if($ret){
                Helper_Json::formJson('别名已存在,请换一个', 'n');
            }
            $result = $this->loadModel('Newscategory',array(),'Admin')->saveData($data, $id);
            $msg = '修改';
        } else {
            //判断别名是否存在
            $ret =$this->loadModel('Newscategory',array(),'Admin')->isTrueName($data['nc_alias']);
            if($ret){
                Helper_Json::formJson('别名已存在,请换一个', 'n');
            }

            $data['nc_addtime'] = date('Y-m-d H:i:s');
            $result = $this->loadModel('Newscategory',array(),'Admin')->saveData($data);
            $msg = '添加';
        }
