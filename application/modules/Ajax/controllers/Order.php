...
//        $this->isLogin();
    ...
    public function expressSaveAction() {
        ...
        //商品信息
        $row    = $this->loadModel('Express', array(), 'Mall')->getInfo($id);
        if(empty($row)){
            Helper_Json::formJson('物流信息不存在');
        }

        $data = array(
            'oe_express_company' => $post['o_express_company'],
            'oe_express_number'  => $post['o_express_number'],
        );

        $ret = $this->loadModel('Express', array(), 'Mall')->saveData($data, $id);
        if($ret){
            //订单进度
            $result = array(
                'o_id'       => $id,
...
//
//            $openId = $this->loadModel('Member', array(), 'Member')->getOpenId(intval($row['m_id']));
