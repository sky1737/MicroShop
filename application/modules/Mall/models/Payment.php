...
class PaymentModel extends BaseModel {
    ...
    public function __construct(){
        parent::__construct('www', 't_payment', 'p_id');
        $this->_field = 'p_id, m_id,p_type,p_order_no,p_transaction_no,p_money,p_addtime';
    }
    ...
    private function _setWhereSQL ($where = array()) {
        if (isset($where['orderSn']) AND $where['orderSn']) {
            $this->_db->where("p_order_no", $where['orderSn']);
        }

        if (isset($where['endTime']) AND $where['endTime']) {
            $this->_db->where("p_addtime  <= '" . $where['endTime'] . "'");
        }

        if(isset($where['startTime']) AND $where['startTime']) {
            $this->_db->where("p_addtime  >= '" . $where['startTime'] . "'");
        }
