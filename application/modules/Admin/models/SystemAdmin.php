...
class SystemAdminModel extends BaseModel {
    ...
    {
        parent::__construct('www', 't_system_admin', 'a_id');
    }
    ...
    private function addLoginLog($userInfo) {
        ...
        return $this->_db->insert('t_system_loginLog')->rows($data)->execute();
    }
	
	public function getLogList($aid, $limit=10) {
        $rows = $this->_db->select('ll_username,ll_realname,ll_login_ip,ll_login_time')
                    ->from('t_system_loginLog')
                    ->where('ll_aid', $aid)
                    ->limit($limit)
                    ->order("ll_id", "DESC")
                    ->fetchAll();
    ...
    public function setMd5Password($password) {
        return md5(SystemAdminModel::AUTH_KEY . $password);
    }
    ...
    public function getInfo($id) {
        return $this->_db->select('a_id,a_username,a_realname,a_phone,a_openId,a_status')->from($this->_table)->where($this->_primary_key, $id)->fetchOne();
    }
    ...
    public function getUserPassword($aid) {
        return $this->_db->select('a_password')->from($this->_table)->where('a_id', $aid)->fetchValue();
    }

    public function updatePassword($aid, $data) {
        return $this->_db->update($this->_table)->rows($data)->where('a_id', $aid)->execute();
    }
}