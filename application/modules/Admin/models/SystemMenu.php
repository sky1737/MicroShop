...
class SystemMenuModel extends BaseModel {
    ...
    public function __construct() {
        parent::__construct('www', 't_system_menus', 'm_id');
        $this->_rightDb    =  $this->loadModel('SystemRight', array(), 'Admin');
    ...
    public function getMenuById($id) {
        return  $this->_db->select('m_id','m_parent_id','m_name','m_url','m_tag','m_status','m_addtime','m_order','m_memo','m_module_name','m_controller_name')
                            ->from('t_system_menus')
                            ->where('m_status = 1')
                            ->where('m_id', $id)
                            ->fetchOne();
    ...
    public function saveMenu($data, $id=0){
        if ( $id > 0 ) {
            return $this->_db->update('t_system_menus')->rows($data)->where('m_id', $id)->execute();
        } else {
            return $this->_db->insert('t_system_menus')->rows($data)->execute();
        }
    }
    ...
    public function mergeMenuUrl($data) {
        ...
        $menus = $this->_db->select('m_id', 'm_url')->from('t_system_menus')->where("m_id IN({$ids})")->fetchAll();
        if ( $menus === FALSE )
            return $data;
     ...
     * 我的快捷菜单
     */
    public function myQuickMemu() {
        $this->_db->select('m_id','m_mergeName')
            ->from('t_quick_memu')
            ->where('u_id',A_ID);
        ...
        $sql = "select `m_id`,`m_name`,`m_url`,`m_tag` from t_menus where `m_id` IN ({$ids})";
        $memu = $this->_db->query($sql)->rows;
        $memu = Helper_Array::setIdsKey($memu,'m_id');
        foreach ($data as &$row) {
            if( isset($memu[$row['m_id']]) ) {
                $row['m_name'] = $memu[$row['m_id']]['m_name'];
                $row['m_url'] = $memu[$row['m_id']]['m_url'];
                $row['m_tag'] = $memu[$row['m_id']]['m_tag'];
            }
        }
    ...
    public function getRightMenuList($id=1) {
        $rightMenu = $this->_rightDb->getRight(A_ID);
        $rightMenu = explode(',', $rightMenu);
        $menuList = $this->getMenuList($id);
        if (isset($menuList['son']) ){
            if($menuList['son']){
                foreach ( $menuList['son'] AS $parent_id => $menu ) {
                    if ( !in_array( $menu['m_id'] , $rightMenu ) ) {
                        unset( $menuList['son'][$parent_id] );
                    }
                }
            }
        }
    ...
    public function getMidByName($name) {
        return $this->_db->select('m_id')->from('t_system_menus')->where('m_name', $name)->fetchValue();
    }

    public function getMidByUrl($url){
        return $this->_db->select('m_id')->from('t_system_menus')->where('m_url', $url)->fetchValue();
    }
    ...
    public function deleteMenu($id) {
        return $this->_db->update('t_system_menus')->rows( array('m_status' => 2) )->where("m_id", $id)->execute();
    }
}