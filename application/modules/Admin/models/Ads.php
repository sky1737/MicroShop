...
class AdsModel extends BaseModel {
     ...
     * @return string
     */
    public function getAdgroup($cname){
        $ids = $this->_db->select('a_ids')->from('t_ad_group')->where('ag_cname', $cname)->fetchValue();
        ...
        $data = $this->_db->select('a_id', 'a_title', 'a_alias','a_picture', 'a_link')
            ->from('t_ad')->where("a_id IN($ids)")->where('a_status = 1')->fetchAll();

        //数据集空
        if ( empty($data) ) return FALSE;
//
//        foreach ($data AS &$row) {
//            $row['a_picture']      = $row['a_picture'];
//        }

        $data = Helper_Array::setIdsKey($data, 'a_id');
        //排序
        $idsArr = explode(',' , $ids);
        $rows  = array();
